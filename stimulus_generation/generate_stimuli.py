...
def sample_s1_s2_threshold(samples,frange,dfrange,threshold):
    '''
    Sampling frequency pairs, first one according to uniform
    Second one conditional on first setting a difficulty sampled from  U[-df,df]
    with resampling if out of range bounds
    ...
    for inner in (-1, 1):
        ...
        for ii in range(1,samples):
            ...
            if (np.abs(dist) < threshold):
                s2[ii] = s1[ii] + inner*direction*(np.random.rand()*(dfrange[1]-dfrange[0]) + dfrange[0])
            else:
                s2[ii] = s1[ii] - inner*direction*(np.random.rand()*(dfrange[1]-dfrange[0]) + dfrange[0])
    ...
    return s1_pm, s2_pm, s1_mp, s2_mp

def sample_s1_s2_markov(samples, frange, dfrange, mrange_close, mrange_far, proportions):
    '''
    Sampling frequency pairs, first one according to uniform
    Second one conditional on first setting a difficulty sampled from  U[-df,df]
    with resampling if out of range bounds
